...
- **Frontend:** Events page:
  ...
  - **Ticket count:** For Gatewav events, the card shows an accurate sold/capacity count (e.g. **3/100 tickets**) when the Ticketing feed returns capacity and a sold count (see below). JOSCITY events still show "X / Y going" from local going count.
...
## Feed fields for accurate ticket count (e.g. 3/100)

For the Events page to show the correct sold/capacity (e.g. **3/100 tickets**) for Gatewav events, the Ticketing feed (`GET .../api/events/feed/joscity`) should include per event:

- **Capacity (max):** `event_capacity`, `capacity`, `max_capacity`, or `max_attendees`
- **Sold count:** `tickets_sold`, `sold`, `sold_count`, `attendees_count`, `going`, `event_registered`, `registered`, `attendees`, `current_attendees`, `booked`, or `total_sold`

If the Ticketing API uses different property names, add them to the backend mapping in `New_Joscity/controller/eventsController.js` inside `fetchTicketingEvents`.
