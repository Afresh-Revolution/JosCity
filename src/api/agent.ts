...
export type AgentProfile = {
  ...
  star_level?: {
    ...
    levels?: Array<{ level: number; label: string; jobs_required: number; can_withdraw: boolean; reached?: boolean; current?: boolean }>;
  };
  distance_km?: number;
};
export type CatalogueItem = {
  item_id: number; agent_user_id: number; title: string; description?: string; category_slug?: string;
  product_price: number; agent_fee_percent: number; fee_amount: number; total_price: number; images?: string[]; status: string;
  source_kind?: 'joscity' | 'external'; source_label?: string; source_name?: string; source_url?: string; listing_id?: number | null;
};
export type CatalogueSourceListing = {
  listing_id: number; title: string; description?: string; price: number; image_url?: string;
  category?: string; seller_user_id?: number; business_name: string;
};
export type AgentRequest = {
  request_id: number; title?: string; description?: string; package_description?: string;
  pickup_address?: string; destination_address?: string; status: string; target_budget?: number;
  images?: string[]; source_type?: Service; requester_user_id?: number; agent_user_id?: number;
};
...
export type AgentReview = {
  review_id: number; job_id?: number; rating: number; comment: string; created_at: string;
  reviewer_name: string; reviewer_picture?: string | null;
};
export type Job = {
  job_id: number;
  source_type: Service;
...
export function requestKey(request: AgentRequest, fallback: Service = 'buy') {
  return `${request.source_type || fallback}-${request.request_id}`;
}

export function serviceOf(request: AgentRequest, fallback: Service = 'buy'): Service {
  return request.source_type === 'delivery' ? 'delivery' : request.source_type === 'buy' ? 'buy' : fallback;
}
