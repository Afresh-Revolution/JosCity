...
const StoriesSection: React.FC<StoriesSectionProps> = ({
  ...
  const handleStoryClick = (story: Story) => {
    ...
    const userStories = storiesByUser.get(story.userName) || [story];
    // Filter out expired stories
    const validStories = userStories.filter((s) => {
    ...
    <div className="newsfeed-stories">
      <div className="newsfeed-stories__container">
        ...
        {filteredStories.map((story) => {
          ...
          const shouldShowContentPreview =
            story.type === "text" && story.content;
          const shouldShowInitials =
            !story.avatar || story.avatar.trim() === "";
          const getInitialsFromName = (name: string): string => {
            const parts = name.trim().split(" ");
            if (parts.length >= 2) {
              return (
            ...
            <div
              ...
              <div className="newsfeed-stories__avatar-wrapper">
                {shouldShowContentPreview ? (
                  <div className="newsfeed-stories__avatar newsfeed-stories__avatar--content">
      ...
      {viewingStories.length > 0 &&
        currentStoryIndex < viewingStories.length && (
          <StoryViewer
            stories={viewingStories}
            currentIndex={currentStoryIndex}
            onNavigate={(newIndex) => setCurrentStoryIndex(newIndex)}
            onClose={() => {
              setViewingStories([]);
              setCurrentStoryIndex(0);
            }}
